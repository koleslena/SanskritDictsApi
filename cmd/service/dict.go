...
func newDict(pathDictName string) (*Dict, error) {
...
func (d *Dict) GetSuggestions(term string, limit int) ([]KeyData, error) {
	return d.loadData(fmt.Sprintf("SELECT * from mwkeys m where m.key like '%s%%' order by lnum LIMIT %d", term, limit))
}

func (d *Dict) GetSuggestion(term string) ([]KeyData, error) {
	return d.loadData(fmt.Sprintf("SELECT * from mwkeys m where m.key like '%s' order by lnum LIMIT %d", term, 1))
}

func (d *Dict) GetResult(nums string) ([]KeyData, error) {
	return d.loadData(fmt.Sprintf("SELECT * from mw m where m.lnum in (%s)", nums))
}

func (d *Dict) GetResultForNum(term string) ([]KeyData, error) {
	return d.loadData(fmt.Sprintf("SELECT * from mw m where m.key like '%s' order by lnum LIMIT 1", term))
}

func (d *Dict) GetResultList(lnum float32, count float32) ([]string, error) {
	sprintf := fmt.Sprintf("SELECT m.key from mw m where m.lnum >= %f and m.lnum <= %f group by m.key order by m.lnum ", lnum-count, lnum+count)
